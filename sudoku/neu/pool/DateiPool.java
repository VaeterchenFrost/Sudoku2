...
public class DateiPool implements Pool0 {
	...
	/**
	 * S�ubert das Topf-Verzeichnis entsprechend AblageVorschrift.
	 * Nicht-Sudokus werden ebenfalls gel�scht.
	...
	public Boolean setze(NeuTyp neuTyp, InfoSudoku sudoku, int loesungsZeit) {
		...
		synchronized (this) {
			...
			if (new NeuTyp(Typ.VOLL).equals(neuTyp)) {
				...
				// Bearbeitung der T�pfe mit Schwierigkeiten
				Schwierigkeit schwierigkeit = neuTyp.gibWieSchwer();

				// Nur gerade L�sungszeiten speichern?
				AblageVorschrift ablageVorschrift = Pool0.AblageVorschrift.gibVorschrift(schwierigkeit);
				...
				if (f.exists()) {
					// Je L�sungszeit eine Datei zur Verdopplung speichern?
					...
					if (dateiName == null) {
						String bemerkung = String.format("Dateien f�r die L�sungszeit %d existieren alle",
								loesungsZeit);
