...
public class SudokuSucher { // implements LangerProzess{
	...
	private static StrichListe gibLinienStriche(List<Sprung> spruenge, String titel, String linienName) {
		...
		// Die beiden letzten als relevant erkannten Spr�nge
		Sprung weissSchwarzSprung = null; // Sprungh�he < 0
		Sprung schwarzWeissSprung = null; // Sprungh�he > 0
	 ...
	 * 			Die bereits erkannten Striche der einen Richtung werden �bernommen. 
	 * 			Sie werden nicht neu erstellt!  
	 * @param minStrichLaengeSenkrecht
	 * 			Die kleinste erlaubte L�nge eines senkrechten Striches 
	 * @param minStrichLaengeWaagerecht
	 * 			Die kleinste erlaubte L�nge eines waagerechten Striches 
	 * @param titel 
	...
	private static StricheKreuz gibStriche(LinienWeiss linienWeiss, StricheKreuz stricheKreuz,
		...
		if (spaltenErstellen) {
			systemOut(String.format("%s Spalten-Strich-Erstellung mit Min-Strich-L�nge=%d Pixel", titel,
		...
		if (zeilenErstellen) {
			systemOut(String.format("%s Zeilen-Strich-Erstellung mit Min-Strich-L�nge=%d Pixel", titel,
	...
	public void prozess() {
		...
		// Mehrere Suchl�ufe mit immer k�rzeren erlaubten Strichen
	 ...
	 * @param nSuchLauf Eine gr��ere Zahl verkleinert die geforderte Strichl�nge 
	...
	private boolean suchen(LinienWeiss linienWeiss, int nSuchLauf) {
		systemOut(String.format("%s.suchen(): Untersuchung Gesamtbild mit Breite=%d und H�he=%d, Lauf %d",
				SudokuSucher.class.getName(), image.getWidth(), image.getHeight(), nSuchLauf));
		systemOut(" ==========================================================================");
		returnImage = image;

		// Erstellung der minimal geforderten Strichl�ngen:
		// Es wird davon ausgegangen, dass auf der schmalen Bildseite max 3 Sudokus liegen,
		// auf der langen Seite max 6 Sudokus.
		// Die Zwischenr�ume zwischen den Sudokus ber�cksichtigen heisst: jeweils + 1.
		...
		if (stricheKreuz.istOK()) {
			...
			// Es wird vorausgesetzt, dass s�mtliche Striche richtige Sudoku-Striche
			// BildFeldGruppen (je 1 f�r 1 Sudoku) erstellen
			...
			// Das Sudoku muss schon ordentlich gro� auf dem Monitor sein, damit die Zahlen sicher erkannt werden.
			// Deswegen ist der Versuch mit dem Bildausschnitt nicht n�tig.
			...
			// //stricheKreuz: die bekannten Striche m�ssen jetzt auf den Ausschnitt transformiert werden!
			...
			// } else {
			// systemOut("Kein Gl�ck gehabt mit Ausschnitt !?");
			// }
			// } else {
			// da l��t sich nichts machen
			systemOut("Kein Gl�ck gehabt !?");
			// }
		} // if (stricheKreuz.istOK()) {
		return sudokus != null;
	}
