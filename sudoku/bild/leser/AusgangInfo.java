...
class AusgangInfo {
	 ...
	 * @param rechteck
	 * @param istOben
	 * @return null oder Startpunkt (etwa Mittelpunkt) in einem wei�en Feld mit schwarzer (teilweiser) Umrandung
	 */
	 ...
	 * @param y die noch gute Y-Koordinate (auf ihr geht es zur�ck von x zu xEnde)
	 ...
	 * @param image
	 * @param rechteck
	 * @param start Startpunkt f�r die Suche
	...
	static private boolean istAusgang(BufferedImage image, Point start, int xZiel, int yAusweich,
		...
		while (x != xZiel) {
			...
			} else { // Schwarz: Gibt es eine Ausweichm�glichkeit (in y-Richtung) ?
				y += yAusweich;
				x -= xRichtung;
				if (istWeiss(image, x, y, istSystemOut)) {
					// Dies ist die Ausweichm�glichkeit
				} else {
					// unter bzw. �ber letztem weiss ist schwarz: Ende oder Sackgasse?
