...
/**
 * @author heroe
 * Dies ist die "heisse" Logik, die aus dem verr�ckten Haufen von Strichen 
...
public class SudokuFinder {
	...
	// Bei 7 Abst�nden k�nnte man annehmen dass 2 nicht gesehen wurden und diese erg�nzbar sind.
	 ...
	 * @param linienName
	 * @return Die Striche, die als Sudoku-Striche erkannt wurden. F�r jedes Sudoku extra.
	...
	/**
	 *  0. Klappt es mit dem n�chsten Abstand bei doppelter Toleranz?
	 *  1. Vielleicht klappt es mit der Summe noch des n�chsten Abstandes weil ein Strich sich eingeschmuggelt hat,
	 ...
	 * @param abstandToleranzProzent Toleranz der Abst�nde in Prozent des Durchschnitts der guten Abst�nde.
	...
	static private StrichListe gibAusWenigenAbstaenden(List<Abstand> guteAbstaende, List<Abstand> alleAbstaende,
		...
		systemOut(String.format("gibAusWenigenAbstaenden() mit %d Start-Abst�nden, Durchschnitt=%d (%d bis %d):",
		...
		while (iZu0.hasNext()) {
			...
			if (istImBereich(testAbstand.gibAbstand(), abstandMin, abstandMax)) {
				...
				if (istImBereich(testAbstand.gibAbstand(), 2 * abstandMin, 2 * abstandMax)) {
					...
					// Vielleicht klappt es mit der Summe der n�chsten Abst�nde weil Striche sich eingeschmuggelt haben
	...
	static private List<Abstand> gibGuteAbstaende(List<Abstand> abstaende, int nGuteMin, int abstandMin,
		...
		// Mehrere gute Abst�nde direkt nebeneinander suchen ab iZu0.
		...
		while (iZu0.hasNext()) {
			...
			if (istGuterAbstand) {
				...
				// Es geht ab hier also nicht mehr weiter mit guten Abst�nden
				if (guteAbstaende.size() < nGuteMin) {
					// Erneut weiterversuchen
					guteAbstaende.clear();
				} else {
					// Dann ist der Auftrag auch erf�llt
					break;
				}
			} // if(istGuterAbstand)
		} // while()
	...
	static private StrichListe gib1StrichGruppe(StrichListe striche, String linienName) {
		...
		// Toleranz der Abst�nde in Prozent des Durchschnitts der guten Abst�nde.
