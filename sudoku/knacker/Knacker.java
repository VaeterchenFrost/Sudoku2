...
public class Knacker {
	...
	/**
	 * @author Hendrick 
	 * 	KEINE Es sollen gar keine Versuche (Eintrag auf Felder mit mehreren m�glichen Zahlen) gestartet werden. 
	 *  EINE Es darf eine VersuchsEbene gestartet werden 
	 *  UNBEGRENZT Es d�rfen beliebig viele Versuche gestartet werden
	 */
	...
	public static String gibText(Ergebnis ergebnis) {
		...
		case UNGELOEST:
			return "Das Sudoku scheint nicht l�sbar";
		case PROBLEM:
	 ...
	 * 
	 * @return Es gibt 3 m�gliche Zust�nde nach treibeKlare():
	 * 		- Problem: Wenn zuvor eine falsche Zahl gesetzt wurde, 
	 * 		- null: Ungel�st: Wenn es keine weiteren Solisten mehr gibt. 
	...
	private Problem treibeKlare(boolean hatZeit, boolean schreibeBericht) throws Exc, SudokuFertig {
		...
		while (true) {
			...
			if (eintrag == null) {
				// F�r diesen Zustand gibt es keine logisch eindeutige L�sung (mit der Klugheit)
				break; // =======================>
			}
	 ...
	 * Es wurde ein Eintrag auf ein Feld mit mehreren m�glichen Zahlen gesetzt.
	...
	private Ergebnis loeseIntern(boolean hatZeit) throws Exc, MeldeTip {
		// Falls das L�sen des Sudoku nicht gelingt: Den Ursprungszustand
		// wiederherstellen
		int markierungsID = setzeMarkierung();
		Ergebnis ergebnis = Ergebnis.ungeloest();
		...
		try {
			...
			do {
				...
				if (optionen.gibMaxErlaubteVersuchsEbene().ordinal() >= VersuchsEbenen.EINE.ordinal()) {
					...
					systemOutZeit("War Versuche 1 Ebene mit 2 m�glichen im Feld");
					...
					// systemOutZeit("War Versuche n Ebenen mit 2 m�glichen im Feld");
					// if (!wurdeEintragGesetzt) {
					// if (feldPaare == null) {
					...
					if ((!wurdeEintragGesetzt)
						...
						if (istFertig) {
							...
							ergebnis = Ergebnis.problem(new Problem("Das Sudoku besitzt keine L�sung"));
						}
