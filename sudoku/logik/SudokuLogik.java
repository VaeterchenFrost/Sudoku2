...
public class SudokuLogik extends FeldMatrix {
	// ab dieser Anzahl an Vorgaben k�nnte das Sudoku ausreichend festgelegt sein
	...
	static private EnumMap<Logik_ID, Logik__Infos> gibLogiken() {
		...
		for (int iLogik = 0; iLogik < logikArray.length; iLogik++) {
			...
			case XYFLUEGEL:
				...
				break;
			default:
				// F�r eine Exception ist es zu dieser (static) Stunde noch ein bischen fr�h. Deshanlb dies unten.
	...
	/**
	 * @author heroe
	 * Enth�lt das Ergebnis von setzeMoegliche(): Entweder ein erkanntes Problem 
	...
	public SudokuLogik(EintragsEbenen eintragsEbenen) {
		...
		// gruppen erstellen: 1. K�sten, 2. Zeile, 3. Spalten
		// Diese Reihenfolge soll der Reihenfolge in Gruppe.Typ entsprechen: F�r die L�sungs-Zeit-Errechnung!
	...
	private Problem sucheGruppenProblem(Gruppe gruppe) {
		...
		//
		// // Wenn eine Zahl n m�gliche Felder hat, m�ssen in diesen n Feldern auch mindestens n Zahlen m�glich sein
	...
	/**
	 * @return Anzahl der K�sten mit einem oder mehr freien Feldern
	 */
	...
	/**
	 * Falls ein freies Feld mit nur einer m�glichen Zahl (durch die Logik) erkannt wird, erh�lt dieses ihren Eintrag.
	 * Das Sudoku muss danach nicht gel�st sein: 
	 *  - Wenn es n�mlich kein eindeutiges ist!
	...
	/**
	 * Solange freie Felder mit nur einer m�glichen Zahl (durch die Logik) erkannt werden, erhalten diese ihren Eintrag.
	 * Das Sudoku muss danach nicht gel�st sein: 
	 *  - Wenn es n�mlich kein eindeutiges ist!
	 *  - Oder die Klugheit nicht ausreicht
	 * @throws Exc
	...
	/**
	 * Die Ansiedelung dieser Funktionalit�t an dieser Stelle hat den bedeutenden Effekt,
	 * dass die Liste der Felder private bleibt!
	 * @return Die Liste von FeldNummerMitZahl, die Felder benennt, die jeweils genau zwei m�gliche Zahlen beinhalten. 
	 * @throws Exc
	 */
	...
	/**
	 * Ziel ist das Erkennen bzw. Erreichen eines freien Feldes mit nur einer m�glichen Zahl: Dass kann dann ein Eintrag werden. 
	 * F�r freie Felder werden die m�glichen Zahlen neu ermittelt:
	 *   1. Jedes Feld setzt seine m�glichen Zahlen
	 *   2. Soweit es die Logik hergibt werden von diesen Zahlen gel�scht, 
	 *   		um Felder mit nur einer m�glichen Zahl zu erreichen. 
	 *   3. Es werden Probleme gesucht.
	 * @param hatZeit Bei true wird die "menschliche" L�sung genutzt 
	 * @param feldPaareSetzen Mit true werden die in jedem Feld gesetzen FeldPaare angefordert
	 * @return SetzeMoeglicheErgebnis: Mit einem von 4 denkbaren Zust�nden:
	 * 					1. Mit dem zu setzenden Eintrag oder
	 * 					2. Mit einem erkannten Problem wenn das Sudoku eine falsche Zahl beinhaltet oder
	 * 					- Leeres Ergebnis 
	 * 						3. wenn die L�sung nicht weiter vorangetrieben werden kann oder
	 * 						4. das Sudoku bereits vollst�ndig gel�st war. 
	 * @throws Exc
	 */
	public SetzeMoeglicheErgebnis setzeMoegliche(Klugheit klugheit, boolean hatZeit, boolean feldPaareSetzen)
			throws Exc {
		// Der Schreiber l�uft nur wenn er gebraucht wird:
		...
		// Jedes freie Feld ermittelt die M�glichen anhand seiner Gruppe (Zeile+Spalte+Kasten) neu
		...
		while (true) {
			...
			if (null != eintrag) {
				// Das Erkennen eines freien klaren Feldes (mit nur einer m�glichen Zahl) war das Ziel
				break;
			}
			if (anzahlGeloeschterZahlen == 0) {
				// Es gibt keine Erfolge mehr beim L�schen von (Un-) M�glichen
				break;
			}
			...
			for (int iLogik = 0; iLogik < logiken.length; iLogik++) {
				...
				if (klugheit.istSoKlug(logik.gibLogikID())) {
					...
					if (logikErgebnis != null) {
						// Tip setzen
						if (tipKurier != null) {
							// Es gibt Logikl�ufe, die gar keine entsprechende Logik-Situation vorfinden!
						...
						// Das L�schen der vorgeschlagenen m�glichen Zahlen realisieren
						...
						if (logikErgebnis.eintrag != null) {
							...
							// Ein f�r einen Eintrag vorgesehenes Feld besitzt im Allgemeinen mehr als nur eine m�gliche Zahl:
							// Z.B. sieht Logik O1 in einer Gruppe die 8 einzig im einem Feld.
							// Da k�nnen in diesem Feld noch andere m�gliche Zahlen drin sein.
							// Es muss noch daf�r gesorgt werden, dass dieses Feld nur einzig die Eintrags-Zahl
							// als m�gliche Zahl besitzt:
						...
						// Bewertung der gel�schten Zahlen
						if (logikErgebnis.loeschZahlen != null) {
							anzahlGeloeschterZahlen = logikErgebnis.loeschZahlen.size();
							if (anzahlGeloeschterZahlen > 0) {
								// Nach gel�schten Zahlen soll wieder ab der einfachsten Logik begonnen werden
								break; // for
							}
						}
					} // if (logikErgebnis != null) {
