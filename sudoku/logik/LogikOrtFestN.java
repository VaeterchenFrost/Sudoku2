...
class LogikOrtFestN implements Logik__Interface {
	...
	/**
	 * @param anzahlZahlen
	 * @return Die id f�r diese Logik. Diese Logiken m�ssen im enum Logik nicht hintereinander stehen! 
	 */
	...
	/**
	 * Erkennt und setzt M�glichGeschwister, n�mlich n Zahlen in n  Feldern in einer Gruppe: 
	 * Eine Zahl ist in ZWEI Feldern der Gruppe m�glich: Wenn es EINE weitere Zahl mit genau diesen Feldern gibt:
	 *     		=> Die M�glichen dieser Felder erhalten nur diese ZWEI Zahlen.
	 * Eine Zahl ist in DREI Feldern der Gruppe m�glich: Wenn es ZWEI weitere Zahlen mit genau diesen Feldern gibt:
	 *     		=> Die M�glichen dieser Felder erhalten nur diese DREI Zahlen.
	 * u.s.w.
	 * M�glichGeschwister ist allerdings nur, wer mindestens 1 m�gliche Zahl l�scht.
	 * @param gruppe
	 * @param anzahlFelder  oder 3 (Felder in der Gruppe mit genau  bzw. 3 gleichen m�glichen Zahlen
	...
	static private Geschwister gibMoeglichGeschwister(Gruppe gruppe, int anzahlFelder) throws Exc {
		...
		// 1. Ob sie in genau anzahlFelder m�glich ist und
		// . ob es eine weitere Zahl (bzw. weitere bei anzahlFelder=3) gibt, die in genau denselben Feldern m�glich ist wie die basisZahl.
		for (int basisZahl = 1; basisZahl < 10; basisZahl++) {

			// Sicht auf die Gruppe erschaffen
			// Der Parameter '1':
			// Nach dem Lauf der Logik OrtFest1 k�nnen weitere m�gliche Zahlen so gel�scht worden sein,
			// dass eine m�gliche Zahl als einzige im Feld steht. Auch diese ist hier relevant!
			ZahlenFeldNummern moeglicheFelderJeZahl = gruppe.gibFelderJeMoeglicheZahl(1);

			// Die freien Felder dieser m�glichen basisZahl
			FeldNummerListe basisZahlFelder = moeglicheFelderJeZahl.gibFeldNummern(basisZahl);
			if (basisZahlFelder != null) {
				// Die Anzahl der freien Felder dieser m�glichen basisZahl
				int basisZahlFeldAnzahl = basisZahlFelder.size();

				// Wenn Anzahl der freien Felder dieser m�glichen basisZahl genau anzahlFelder ist:
				// K�nnte (!) es sich um gesuchte Geschwister handeln
				if (basisZahlFeldAnzahl == anzahlFelder) {
					...
					// Also die Zahlen, die in genau denselben Feldern und nur dort m�glich sind wie die basisZahl.
					...
					// Wenn anzahlFelder-1 == Geschwister-Anzahl ist: Es handelt sich um M�glich-Geschwister.
					// "-1" weil die basisZahl selbst ja eine der endg�ltigen Geschwisterzahlen ist.
					int braucheGeschwisterAnzahl = anzahlFelder - 1;
					if (braucheGeschwisterAnzahl == geschwisterZahlen.size()) {
						// Geschwister sind gefunden:
						// Wieviele m�gliche Zahlen besitzen die Geschwister (basisZahlFelder)?
						...
						if (moegliche.gibAnzahlVorhandene() > anzahlFelder) {
							// Wenn mehr als zwei M�gliche existieren wird gehandelt
							// Geschwister als M�gliche um die basisZahl erg�nzen
							geschwisterZahlen.add(0, new Integer(basisZahl));

							// Die zu l�schenden Zahlen vermerken
							...
							if (!loeschZahlen.isEmpty()) {
								...
								// // Diese Geschwisterzahlen einzig als M�gliche stehen lassen in ihren Feldern
	...
	private class TipInfoOrtFestN extends TipInfo0 {
		 ...
		 * @param solistErgebnis null oder ein �ber die Geschwister-Felder gefundener Solist mit dessen Gruppe
		 * @param infoSudoku
		 */
	...
	public LogikErgebnis laufen(boolean hatZeit, boolean istTip, List<TipInfo> ignorierTips) throws Exc {
		...
		if (!freieGruppen.isEmpty()) {
			...
			for (int i = 0; i < freieGruppen.size(); i++) {
				...
				// Gibt es n Zahlen, die in genau n Feldern m�glich sind?
