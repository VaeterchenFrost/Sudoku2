...
class Logik_Kasten2 extends LogikKastenN {
	...
	public String[] gibErgebnis() {
		return new String[] { "Im Kasten wird die Zahl auf diesen 2 Linien aus den m�glichen dieser Felder gel�scht." };
	}
	...
	private KastenErgebnis beachteNachbarn(Kasten kasten, boolean istSpalte) throws Exc {
		...
		for (int basisZahl = 1; basisZahl < 10; basisZahl++) {
			// F�r jeden der beiden Nachbarn (0 und 1) dessen Linien bereitstellen
			LinienListe nachbar0Linien = linienDerNachbarn.get(0);
			LinienListe nachbar1Linien = linienDerNachbarn.get(1);

			// Die Indizees der Linien der beiden Nachbarn ermitteln, die Zahl als M�gliche enthalten
			...
			if (gleichePaarLinien) {
				...
				for (int iLinieDerZahl = 0; iLinieDerZahl < iLinien0DerZahl.size(); iLinieDerZahl++) {
					...
					// Meine Felder auf dieser Linie, in denen die (m�gliche) Zahl gel�scht wird.
				...
				// Wenn in einer meiner beiden Linien eine m�gliche Zahl gel�scht wurde war diese Logik aktiv:
