 ...
 *  1. Es geht einzig und allein um Felder, die 2 m�gliche Zahlen besitzen.
 *  2. Diese Felder sind �ber eine gemeinsame Zahl miteinander  innerhalb einer Gruppe verbunden.
 * Die Forderung der Feldpaare, dass eine m�gliche Zahl nur zweimal je Gruppe auftreten darf, gibt es hier jedenfalls nicht!
 * 
 * Diese Logik sucht f�r einen Tip die k�rzeste Kette. Im Falle, dass die Logik nicht f�r einen Tip l�uft,
 * nutzt sie die erste beste gefundene Kette. Dadurch kann es (meist) sein, dass in beiden F�llen 
 * verschiedene m�gliche Zahlen (bzw. vorgeschlagene Eintr�ge) als Ergebnis erscheinen, wass ein bischen irretieren k�nnte. 
 * Aber der L�sung des Sudoku schadet das garnichts!
 */
class Logik_Auswirkungskette implements Logik__Interface {
	 ...
	 * 				Jedes Kettenglied benennt die Nummer des Feldes und die Zahl, die hierher gef�hrt hat.
					Das erste Kettenglied (auf Index 0) besitzt die Zahl, die dann der Eintrag werden m�sste.
	 * @param feldPartner Alle Partner von Feldern, die als ein Kettenglied infrage kommen k�nnten
	 * @param sudoku Gibt Felder f�r FeldNummern
	...
	static private ZahlenListe gibKette(ZahlenListe vorhandeneKette, Map<FeldNummer, ZahlenFeldNummern> feldPartner)
		...
		// denn die beiden Nachbar-Felder des Ende-Feldes m�ssen dieselbe Zahl als Eintrag erhalten.
		// Damit k�nnen die beiden Nachbar-Felder nicht innerhalb eines Kastens liegen.
		...
		// Wir k�nnen hier �ber ein FeldPaar auf einem Feld gelandet sein, dass nicht genau zwei m�gliche Zahlen besitzt!
		// ArrayList<Integer> moegliche = aktuellesFeld.gibMoegliche();
		// if (moegliche.size() != 2) {
		// systemOut(vorhandeneKette, String.format(
		// "Vorhandene Kette f�hrt auf eine Feld, das nicht genau 2 m�gliche Zahlen besitzt, sondern %d",
		// moegliche.size()));
		// return null;
		// }

		// Die Zahl von den genau 2 m�glichen Zahlen, die auf dem Weg weiterf�hren k�nnte, ermitteln.
			...
			if (wegZahl != herkunftsZahl) {
				...
				if (zielFeldNummern != null) {
					for (FeldNummer zielFeldNummer : zielFeldNummern) {
						...
						// systemOut(String.format(" Idee f�r TestZiel %s", testZiel));
		...
		systemOut(vorhandeneKette, true, "Vorhandene Kette f�hrt mit nicht weiter");
		return null;
	}

	/**
	 * @param feldPartner Alle Felder mit ihren Partnern, die jeweils in einer Kette sein k�nnten.
	 * @param gibKuerzesteKette bei true wird die k�rzestes Kette gesucht
	 * @param sudoku Gibt Felder f�r FeldNummern
	 * @return Eine Kette oder null, falls keine gefunden werden konnte.
	 * 				Die Kette startet und endet auf demselben Feld mit derselben Zahl. 
	 * 				Die andere m�gliche Zahl des Star- bzw. Ende-Feldes ist dann schon der Vorschlag f�r einen Eintrag:
	 * 				Egal wie die Ketten-Felder, die weder Start noch Ende sind, belegt sind: 
	 * 				Die andere m�gliche Zahl muss im Start- bzw. Ende-Feld gesetzt sein, wenn das Sudoku l�sbar ist. 
	 * @throws Exc 
	 */
	...
	/**
	 * @author heroe
	 * Die �bergebenen Felder sind alles Felder der einen Gruppe, die alle die eine m�gliche Zahl besitzen.
	 * Die �bergebenen Felder werden zu FeldPaaren kombiniert, wenn sie jeweils genau 2 m�gliche Zahlen enthalten.
	 */
	...
	public String[] gibSituation() {
		return new String[] { "Die Feldpaare bilden eine Kette. die geschlossen ist:",
				"- 'Normale' Kettenfelder liegen mit beiden m�glichen Zahlen in der Kette.",
				"- Das Verschlu�feld liegt nur mit einer m�glichen Zahl in der Kette.",
				"�ber diese eine m�gliche Zahl ist das Verschlussfeld Feldpaar zu seinen beiden Ketten-Nachbar-Feldern." };
	}

	@Override
	public String[] gibErgebnis() {
		return new String[] { "Die nicht verkette m�gliche Zahl des Verschlu�feldes ist ein Eintrag." };
	}
	...
	public LogikErgebnis laufen(boolean hatZeit, boolean istTip, List<TipInfo> ignorierTips) throws Exc {
		...
		// Die Ketten-FeldPaare, die paareweise �ber eine Zahl (in einer Gruppe) verbunden sind.
		// Jedes der Felder besitzt genau zwei m�gliche Zahlen.
		...
		// systemOut(moeglicheKettenEnden, "M�gliche Ketten-End-Felder");
		//
		...
		if (kette != null) {
			...
			// Hier kann man wohl nur eine einzige pauschale Zeit f�r diese Logik annehmen.
			// Die k�nnte h�chstens von der Anzahl der freien Felder abh�ngen.
