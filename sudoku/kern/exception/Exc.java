...
public class Exc extends Exception {
	...
	static public Exc setzeMoeglicheNicht(ArrayList<Integer> moegliche, ArrayList<Integer> neueMoegliche) {
		String text = String.format("setze M�gliche nicht: meine=%s, neue=%s", moegliche, neueMoegliche);
	...
	static public Exc protokollMarkierungIstOhneNachfolger(int markierungId, int markierungKursor,
			int anzahlEintraege) {
		String text = String.format("Protokoll-Markierung (id=%d, kursor=%d) ist ohne Nachfolger: Anzahl Eintr�ge=%d",
	...
	static public Exc setzeVorgabeNurOhneEintraege(Feld feld, int vorgabe, int aktuelleEbene) {
		String text = String.format("%s: Setze Vorgabe %d nur ohne Eintr�ge (Ebene=%d)", feld.gibName(), vorgabe,
				aktuelleEbene);
	...
	static public Exc loescheEintragNichtAufEbene(Feld feld, int aktuelleEbene) {
		String text = String.format("%s: L�sche Eintrag %d nicht: EintragsEbene=%d aktuelle Ebene=%d", feld.gibName(),
	...
	static public Exc setzeEintragNichtOhneMoegliche(Feld feld, int zahl) {
		String text = String.format("%s: Setze Eintrag %d nicht ohne m�gliche Zahlen", feld.gibName(), zahl,
				feld.gibVorgabe());
	...
	static public Exc setzeEintragNichtOhneDieseMoegliche(Feld feld, int zahl) {
		String text = String.format("%s: Setze Eintrag %d nicht: Ist nicht unter den M�glichen %s", feld.gibName(),
	...
	static public Exc ebeneSpieltNichtMit(Feld feld, Eintrag neuerEintrag, int neueEbene) {
		String text = String.format("%s (M�gliche=%s): EintragsObjekt (Ebene=%d, Zahl=%d) erzeugt falsche Ebene=%d",
	...
	static public Exc aktionnurOhneEintrag(String aktionsName) {
		String text = String.format("Die Aktion '%s' ist nur im Sudoku ohne Eintr�ge m�glich", aktionsName);
		return new Exc(text);
	}
